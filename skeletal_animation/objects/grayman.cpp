...
bool grayman::init(){
    ...
    m_world = rt_rotate(float(_radians(-90.0f)),_vec3(1.0f,0.0f,0.0f));
    //**********************************
...
bool grayman::update(){
    ...
    app_throw_hr(_fx->SetTechnique(_api_manager->m_htech));
    ...
    for(uint32_t i=0;i<m_mesh.m_submeshes.m_count;i++){
        ...
        UINT numPasses = 0;
        app_throw_hr(_fx->Begin(&numPasses, 0));
        for(UINT ii = 0; ii < numPasses; ++ii) {
            app_throw_hr(_fx->BeginPass(ii));
