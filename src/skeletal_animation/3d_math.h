...
_matrix4<T> _lookatlh(const _vector3<T>& eye, const _vector3<T>& center, const _vector3<T>& up ) {
    const _vector3<T> f(_normalize(center - eye));
    const _vector3<T> s(_normalize(_cross(up, f)));
    const _vector3<T> u(_cross(f, s));
